...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (isRunning) {
      ...
      ws.onmessage = (e) => {
        ...
        setPrevPrice(priceRef.current); setPrice(p); priceRef.current = p;
      };
    }
    return () => ws?.close();
  }, [isRunning]);
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      const now = new Date();
      if (isRunning && !isAiLoading && now.getSeconds() === 45) {
        // Mock AI (original had Gemini)
        const txt = "Strong bullish order block alignment. Institutional accumulation likely complete.";
